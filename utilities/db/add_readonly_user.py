...
def add_ro_user(config, ro_user):
    password_for_ro = generate_password(25)
    username_for_ro = f'{config['app']['database']}_ro_{ro_user}'

    host = config['app']['host']
    try:
        host = lookup_read_replica(host)
    except:
        print('Unable to find read replica')

    print(f'Host: {host}')
    print(f'Database: {config['app']['database']}')
    print(f'Readonly Username: {username_for_ro}')
    print(f'Readonly Password: {password_for_ro}')
    info = {
    ...
    try:
        with connection.cursor() as c:
            ...
            c.execute(sql.SQL('GRANT CONNECT ON DATABASE {database} to {username}').format(database=sql.Identifier(config['app']['database']), username=sql.Identifier(username_for_ro)))
    except:
        with connection.cursor() as c:
            c.execute(sql.SQL('ALTER USER {username} WITH PASSWORD %(password)s').format(username=sql.Identifier(username_for_ro)), {'password': password_for_ro})
...
def lookup_read_replica(host):
    instance_id = host.split('.')[0]
    client = boto3.client('rds')
    db_info = client.describe_db_instances(DBInstanceIdentifier=instance_id)
    rr = db_info['DBInstances'][0]['ReadReplicaDBInstanceIdentifiers'][0]
    db_info = client.describe_db_instances(DBInstanceIdentifier=rr)
